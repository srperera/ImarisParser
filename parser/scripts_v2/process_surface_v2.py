...
def run_surface_parser_parallel(
    ...
    Args:
        ...
        surface_ids (Tuple[int], optional): A tuple of specific spot IDs to parse.
                                         Defaults to None.
    """
    if surface_ids and not isinstance(surface_ids, tuple):
        raise TypeError("surface_ids must be a tuple")
    ...
    for data_path, save_dir in data_paths:
        ...
        for file_path in imaris_files:
            ...
            try:
                ...
                valid_objects = get_valid_surfaces(data_path=file_path)

                # if surface_ids are provided, filter valid_surface_ids
                if surface_ids:
                    # Filter to only include user-specified spot IDs
                    valid_objects = [
                        idx for idx in valid_objects if (idx + 1) in surface_ids
                    ]
    ...
    for data_path, save_dir in data_paths:
        ...
        for file_path in imaris_files:
            ...
            try:
                ...
                # Instructions: Modify Here If Duplicating For New Use Case
                # Create new version of get_valid_spot_tracks function
                valid_objects = get_valid_surfaces_with_tracks(data_path=file_path)

                # if surface_ids are provided, filter valid_objects
                if surface_ids:
                    # Filter to only include user-specified spot IDs
                    valid_objects = [
                        idx for idx in valid_objects if (idx + 1) in surface_ids
